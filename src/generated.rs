...
pub mod async_internal {
    //! Async endpoint-wrapper structs (crate-internal; re-exported at the
    //! crate root). Enums, Options, and response structs are shared with the
    //! sync mirror and live at the generated root, imported below.
    use super::*;
...
#[cfg(feature = "sync")]
#[doc(hidden)]
pub mod sync_internal {
    //! Synchronous mirror of the endpoint wrappers (feature `sync`).
    //!
    //! Identical API to the async surface minus `async`/`.await`, backed by
    //! [`crate::api::sync::Client`]. Enums, Options, and response structs
    //! are shared with the async surface via the glob import below.
    use super::*;
    use crate::api::sync::Client;
