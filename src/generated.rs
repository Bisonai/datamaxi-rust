...
/// Root client for the Datamaxi+ API.
///
/// Owns one shared [`Client`] and exposes one accessor per endpoint group
/// (e.g. [`Datamaxi::ticker`]); each returns a group wrapper backed by the
/// same connection pool. Construct with [`Datamaxi::new`],
/// [`Datamaxi::new_with_base_url`], or [`Datamaxi::from_client`].
#[derive(Clone)]
pub struct Datamaxi {
    client: Client,
}
...
impl Datamaxi {
    /// Creates a client from an API key, using the default base URL.
    ...
    /// Creates a client from an API key and a custom base URL.
    ...
    /// Returns the [`Announcements`] endpoint group.
    pub fn announcements(&self) -> Announcements {
        Announcements::from_client(self.client.clone())
    }

    /// Returns the [`CexCandle`] endpoint group.
    pub fn cex_candle(&self) -> CexCandle {
        CexCandle::from_client(self.client.clone())
    }

    /// Returns the [`CexSymbol`] endpoint group.
    pub fn cex_symbol(&self) -> CexSymbol {
        CexSymbol::from_client(self.client.clone())
    }

    /// Returns the [`Forex`] endpoint group.
    pub fn forex(&self) -> Forex {
        Forex::from_client(self.client.clone())
    }

    /// Returns the [`FundingRate`] endpoint group.
    pub fn funding_rate(&self) -> FundingRate {
        FundingRate::from_client(self.client.clone())
    }

    /// Returns the [`IndexPrice`] endpoint group.
    pub fn index_price(&self) -> IndexPrice {
        IndexPrice::from_client(self.client.clone())
    }

    /// Returns the [`Liquidation`] endpoint group.
    pub fn liquidation(&self) -> Liquidation {
        Liquidation::from_client(self.client.clone())
    }

    /// Returns the [`Listing`] endpoint group.
    pub fn listing(&self) -> Listing {
        Listing::from_client(self.client.clone())
    }

    /// Returns the [`MarginBorrow`] endpoint group.
    pub fn margin_borrow(&self) -> MarginBorrow {
        MarginBorrow::from_client(self.client.clone())
    }

    /// Returns the [`NaverTrend`] endpoint group.
    pub fn naver_trend(&self) -> NaverTrend {
        NaverTrend::from_client(self.client.clone())
    }

    /// Returns the [`OpenInterest`] endpoint group.
    pub fn open_interest(&self) -> OpenInterest {
        OpenInterest::from_client(self.client.clone())
    }

    /// Returns the [`Premium`] endpoint group.
    pub fn premium(&self) -> Premium {
        Premium::from_client(self.client.clone())
    }

    /// Returns the [`Telegram`] endpoint group.
    pub fn telegram(&self) -> Telegram {
        Telegram::from_client(self.client.clone())
    }

    /// Returns the [`Ticker`] endpoint group.
    pub fn ticker(&self) -> Ticker {
        Ticker::from_client(self.client.clone())
    }

    /// Returns the [`Token`] endpoint group.
    pub fn token(&self) -> Token {
        Token::from_client(self.client.clone())
    }

    /// Returns the [`TradingFees`] endpoint group.
    pub fn trading_fees(&self) -> TradingFees {
        TradingFees::from_client(self.client.clone())
    }

    /// Returns the [`WalletStatus`] endpoint group.
    pub fn wallet_status(&self) -> WalletStatus {
        WalletStatus::from_client(self.client.clone())
    }
}
